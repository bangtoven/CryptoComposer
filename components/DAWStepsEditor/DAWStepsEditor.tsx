  ...
  return (
    <div className={[css.row, css.header].join(' ')}>
      {emptyArray.map((_, i) => {
        return (
          <div
            className={[css.step, currentStepIndex + 1 === i + stepIndexOffset ? css.stepIsCurrent : ''].join(' ')}
            key={`header-${i}`}
            data-testid={`header`}
          >
...
const DAWStepsEditor: React.FC<Props> = ({
  ...
  const selectedStepNote =
    localSteps[selectedStepIndex] && localSteps[selectedStepIndex].find((s) => s.name === selectedStepNoteName);
  ...
  React.useEffect(() => {
    ...
    if (highestStep) {
      const highestStepIndex = noteNames.findIndex((note) => note === highestStep.name);
  ...
  const handleStepClick = (stepNote, index) => {
    const stepNotes = [...(localSteps[index] ? localSteps[index] : []), stepNote];
    const shouldRemove = stepNotes.filter((s) => s.name === stepNote.name).length >= 2;
    const newStepNotes = shouldRemove ? stepNotes.filter((s) => s.name !== stepNote.name) : stepNotes;
  ...
  return (
    <div className={[css.stepsEditor, className || ''].join(' ')}>
      ...
      <div className={css.steps} ref={stepsRef}>
        ...
        <StepsHeader emptyArray={emptyArray} stepIndexOffset={stepIndexOffset} />
        ...
        {noteNames.reverse().map((noteName, rowIndex) => {
          ...
          return (
            <div className={[css.row, isAccidental ? css.rowIsAccidental : ''].join(' ')} key={noteName}>
              {emptyArray.map((_, columnIndex) => {
                ...
                const currentStepNote =
                  ...
                  localSteps[index].find((stepNote) => stepNote.name === noteName);

                const isCurrent = Boolean(currentStepNote);

                const dataTestId = `step-button-${columnIndex - 1}-${rowIndex}${isCurrent ? '-current' : ''}`;
                ...
                return (
                  <button
                    className={[css.step, isCurrent ? css.stepIsCurrent : ''].join(' ')}
                    onClick={() => {
